...
| V3-F-012 (FEED-04) | Cache HTTP désactivé pour toutes les images | **CODE_PRESENT_UNVERIFIED** (corrigé session 2026-08-23 — cache par défaut restauré dans `CDNAsyncImage`, fidèle à `DiskCacheStrategy.ALL` de tous les chargeurs Glide (`ChargerImages.java`) ; le vrai problème d'origine (403 mis en cache avant l'ajout du header `Referer`) traité de façon ciblée : purge `URLCache` sur toute réponse invalide au lieu de désactiver le cache pour tous ; CI à confirmer) | P2 | `.reloadIgnoringLocalCacheData` trop large (correctif défensif historique jamais restreint) — CORRIGÉ | MEDIUM |
...
| V3-F-019 (BUNNY-03) | Progression upload + mémoire | **CODE_PRESENT_UNVERIFIED** (corrigé session 2026-08-23 — `FeedMediaUploader.uploadVideo` streame maintenant depuis le fichier via `URLSession.upload(for:fromFile:delegate:)` au lieu de `Data(contentsOf:)` préalable ; progression réelle exposée via `UploadProgressDelegate`, fidèle à `ProgressRequestBodyUri.java` (8192o/bloc, `uploaded*100/contentLength`) ; UI mise à jour (`ProgressView(value:)` dans `PublishComposeView`) ; CI à confirmer) | P2 | Pas de % réel, vidéo chargée entière en RAM avant envoi (Android streame par blocs de 8192o) — CORRIGÉ | MEDIUM |
...
| V3-F-025 (CHAT-04) | Mapping des noms d'événements | CODE_PRESENT_UNVERIFIED — réserve de blocage levée (mise à jour de bookkeeping 2026-08-21) : V3-F-016 (connexion socket) est `BUILD_VALIDATED` depuis le Lot 1, donc ce mapping EST maintenant atteignable/vérifiable en pratique ; reste `CODE_PRESENT_UNVERIFIED` faute d'un test réel socket-actif, pas d'un blocage structurel | P1 | Fidèle à la lecture statique | MEDIUM |
