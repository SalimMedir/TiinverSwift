...
| ID | Domaine | Statut | Prio | Écart | Confiance |
|---|---|---|---|---|---|
| V3-F-017 (BUNNY-01) | Métadonnées `activity/add` incomplètes | PARTIAL | P1 | `category`/`metadata`/`template_id`/`consentAi`/`width`/`height`/`video_duration` jamais envoyés, contrairement à un commentaire iOS qui affirme (à tort) que "ce n'est pas envoyé par Android non plus" — catégorie confirmée OBLIGATOIRE côté Android | HIGH |
