...
| V3-F-021 (BUNNY-05) | En-tête `Accept` manquant sur PUT vidéo | **CODE_PRESENT_UNVERIFIED** (corrigé session 2026-08-23 — `Accept: application/json` ajouté à `FeedMediaUploader.uploadVideo`, fidèle à `ActivityService.java:287-292` ; PUT storage/photo confirmée volontairement SANS ce header des deux côtés, pas modifiée ; CI à confirmer) | P2 | Écart HTTP certain, impact incertain — CORRIGÉ | LOW |
...
| V3-F-094 (SILENT-05) | Navigation profil "créateur de la semaine" avec id vide | **CODE_PRESENT_UNVERIFIED** (corrigé session 2026-08-23 — `.disabled(...)` sur les 2 `NavigationLink` de `CreatorOfWeekView`, fidèle au guard `if (idStr==null\|\|idStr.trim().isEmpty()) return;` de `CreatorAdapter.java:59-64` ; CI à confirmer) | P2 | Pas de filet de sécurité pour `isCurrentUser==false` — CORRIGÉ | MEDIUM |
