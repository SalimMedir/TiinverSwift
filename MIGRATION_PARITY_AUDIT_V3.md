...
| V3-F-007 (SEARCH-07) | Tap `#hashtag`/`@mention` dans une légende | **BUILD_VALIDATED** — **doublon confirmé de V3-F-099** (même feature, redécouverte indépendamment par le cycle complémentaire §30.1 sans recoupement d'ID, corrigée `349b606`, CI verte ; `AttributedString.link`/`openURL` jamais exercé sur device, test réel requis) — mise à jour de bookkeeping 2026-08-21, aucun code changé pour cette entrée | **P1** | Fonctionnalité entière absente (pas de `MentionTextView` équivalent) — CORRIGÉ, voir V3-F-099 pour le détail complet | HIGH |
...
| V3-F-011 (FEED-03) | Décodage tableau entier fragile (suggestions) | **BUILD_VALIDATED** (corrigé `f238e8b`, session 2026-08-21 — `SuggestionsRepository.decodeUsers` passé en `compactMap` per-item, même motif que `ChatRepository.decodeMessages`/`FeedRepository.fetchTimeline`, CI verte confirmée 2026-08-23 run `32628912305` — test réel requis) | P2 | `SuggestionsRepository` n'avait pas le motif per-item déjà appliqué ailleurs — CORRIGÉ | MEDIUM |
| V3-F-012 (FEED-04) | Cache HTTP désactivé pour toutes les images | **BUILD_VALIDATED** (corrigé `109cc9b`, session 2026-08-23 — cache par défaut restauré dans `CDNAsyncImage`, fidèle à `DiskCacheStrategy.ALL` de tous les chargeurs Glide (`ChargerImages.java`) ; le vrai problème d'origine (403 mis en cache avant l'ajout du header `Referer`) traité de façon ciblée : purge `URLCache` sur toute réponse invalide au lieu de désactiver le cache pour tous ; CI verte confirmée run `32630694641` — test réel requis) | P2 | `.reloadIgnoringLocalCacheData` trop large (correctif défensif historique jamais restreint) — CORRIGÉ | MEDIUM |
...
| V3-F-019 (BUNNY-03) | Progression upload + mémoire | **BUILD_VALIDATED** (corrigé `109cc9b`, session 2026-08-23 — `FeedMediaUploader.uploadVideo` streame maintenant depuis le fichier via `URLSession.upload(for:fromFile:delegate:)` au lieu de `Data(contentsOf:)` préalable ; progression réelle exposée via `UploadProgressDelegate`, fidèle à `ProgressRequestBodyUri.java` (8192o/bloc, `uploaded*100/contentLength`) ; UI mise à jour (`ProgressView(value:)` dans `PublishComposeView`) ; CI verte confirmée run `32630694641` — test réel requis) | P2 | Pas de % réel, vidéo chargée entière en RAM avant envoi (Android streame par blocs de 8192o) — CORRIGÉ | MEDIUM |
| V3-F-020 (BUNNY-04) | Publication vidéo — chaîne cœur Bunny Stream | COMPLETE_PARITY_CANDIDATE | P2 | Aucun | HIGH |
| V3-F-021 (BUNNY-05) | En-tête `Accept` manquant sur PUT vidéo | **BUILD_VALIDATED** (corrigé `bc7c544`, session 2026-08-23 — `Accept: application/json` ajouté à `FeedMediaUploader.uploadVideo`, fidèle à `ActivityService.java:287-292` ; PUT storage/photo confirmée volontairement SANS ce header des deux côtés, pas modifiée ; CI verte confirmée run `32629372490` — test réel requis) | P2 | Écart HTTP certain, impact incertain — CORRIGÉ | LOW |
...
| V3-F-025 (CHAT-04) | Mapping des noms d'événements | **COMPLETE_PARITY_CANDIDATE** (vérifié exhaustivement session 2026-08-23 — les 35 constantes de `ChatRepository.ROOM` (`ChatRepository.java:107-144`) comparées une par une aux valeurs de `SocketEvent.swift` : TOUTES identiques byte pour byte, y compris la faute d'orthographe réelle préservée `"delivred"` ; aucune divergence trouvée ; test réel d'aller-retour socket requis avant COMPLETE_PARITY_VALIDATED) | P1 | Fidèle à la lecture statique — CONFIRMÉ par comparaison exhaustive, pas seulement une lecture statique isolée | MEDIUM |
...
| V3-F-027 (CHAT-05) | Envoi de message neutralisé par V3-F-016 | CODE_PRESENT_UNVERIFIED — réserve de blocage levée (vérifié session 2026-08-23) : V3-F-016 est `BUILD_VALIDATED` depuis le Lot 1, `socket?.emit(...)` s'exécute donc réellement maintenant. Chaîne `ChatViewModel.send`→`ChatRepository.sendPrivateMessage`/`sendGroupMessage`→`MessagePacket` relue en détail, noms d'événements ET construction de payload confirmés fidèles (y compris un bug Android intentionnellement reproduit dans `updateGroupMessage`, PAS corrigé pour rester compatible avec ce que le serveur reçoit réellement) — aucune divergence trouvée dans le code. Reste `CODE_PRESENT_UNVERIFIED` faute d'un test réel d'aller-retour socket, pas d'un doute sur le code | P2 | Écho optimiste local masque le fait que rien ne part réellement — n'est plus vrai depuis V3-F-016, code vérifié correct | HIGH |
...
| V3-F-042 (GALERIE-11) | Trim vidéo — précision du point de coupe | **BUILD_VALIDATED** — **doublon confirmé de V3-F-124** (même feature, portée élargie/reconfirmée par le cycle complémentaire, corrigée `cd316df`, CI verte ; test réel de comparaison frame-exacte requis avant COMPLETE_PARITY_VALIDATED) — mise à jour de bookkeeping 2026-08-21, aucun code changé pour cette entrée | P1 | `AVAssetExportPresetPassthrough` calait sur le keyframe précédent vs MediaCodec Android, frame-exact — CORRIGÉ (toujours ré-encodé maintenant), voir V3-F-124 pour le détail complet | MEDIUM |
...
| V3-F-055 (AUTH-09) | Google Sign-In | **COMPLETE_PARITY_CANDIDATE** — **doublon confirmé de V3-F-145** (même feature, revérifiée session 2026-08-23 : `GoogleSignInCoordinator.swift` relu en entier, flux fidèle — Credential Manager/`GIDSignIn` → Firebase Auth → lecture `FirebaseUser.uid`/`email`/`displayName`, PAS la réponse Google brute, exactement comme `profile.getUid()` Android ; `clientID`/`serverClientID` confirmés contre le vrai `GoogleService-Info.plist` par une session antérieure, pas devinés) | P2 | Chaîne complète mais jamais testée réellement — CONFIRMÉ CORRECT, test OAuth réel requis | MEDIUM |
...
| V3-F-063 (PROFILE-08) | Lien de bio interne non routé | **BUILD_VALIDATED** (corrigé `36559d2`, session 2026-08-21 — test d'hôte `tiinver.com` avant ouverture, fidèle à `UserProfile.java:458-462`, CI verte confirmée 2026-08-23 run `32628912305` — test réel requis) | P2 | Toujours ouvert en Safari, jamais via `DeepLinkRouter` — CORRIGÉ | MEDIUM |
...
| V3-F-077 (NOTIF-06) | Navigation au tap notification | **COMPLETE_PARITY_CANDIDATE** — **doublon confirmé de V3-F-136** (même feature, requalifiée par le cycle complémentaire §30.8 le 2026-08-20 : `NotificationUtils.java.show()` reconstruit TOUJOURS un `Intent` bare vers `SplashActivity` côté Android, quel que soit le type — routage riche par type mort côté Android lui-même, `getActionDestination()` commenté/jamais appelé. Comportement iOS actuel (`AppDelegate.userNotificationCenter(didReceive:)` → `DeepLinkCenter.shared.route(.notifications)`) déjà à parité, voire meilleur ; mise à jour de bookkeeping 2026-08-23, aucun code changé) | P2 | Bloqué en aval par V3-F-075 pour les messages — **réserve levée, voir V3-F-136** | HIGH |
...
STATUT : **BUILD_VALIDATED** (corrigé `56f62f7`, Phase B, CI confirmée verte — test réel requis avant COMPLETE_PARITY_VALIDATED : publier une photo 1:1/4:3 avec un trait de peinture sur écran ~9:19.5, comparer dimensions/contenu du JPEG uploadé)
...
**P1 majeurs nouveaux — TOUS CORRIGÉS le 2026-08-20 (Phase B), CI verte confirmée sur chacun, tous `BUILD_VALIDATED`** : V3-F-124 (`cd316df`), V3-F-125 (`5eb3358`/`e869825`), V3-F-103 (`c9dd8b1`), V3-F-107 (`c08ce4c`), V3-F-099 (`349b606` — mais `AttributedString.link`/`openURL` jamais exercé sur device, test réel prioritaire), V3-F-102 (`596105f`), V3-F-113 (`ab510b3`), V3-F-114 (`94bd731`/`066fa04` — CI initialement rouge sur `98b8c7d`, erreur de compilation réelle corrigée, pas masquée), V3-F-128 (`9de8647`/`066fa04`), V3-F-129 (`9de8647`/`066fa04`), V3-F-137 (`94bd731`/`066fa04`), V3-F-126 (`56f62f7`, reconfirmation de V3-F-039). V3-F-112 reconfirmé positif sans changement de code (sa réserve "bloqué par V3-F-110" ne tient plus, V3-F-110 est corrigé). **Aucun P1 restant dans la liste explicitement priorisée par cette Phase B.** Backlog P1 plus large (hors liste §30.8 initiale) non passé en revue exhaustive : voir le reste de l'audit V3 pour d'éventuels P1 antérieurs non encore traités.
