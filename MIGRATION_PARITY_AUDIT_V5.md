...
STATUT : BUILD_VALIDATED (2026-08-26) — Option 2 de la RECOMMANDATION appliquée : `RosterRepository.rosterAll()` (Sources/TiinverSwift/Storage/RosterRepository.swift) supprime entièrement la ré-agrégation `MessageEntity` (struct `RosterWithLastMessage`, propriété `messages: CoreDataRepository<MessageEntity>` supprimées) et retourne `[RosterEntity]` brut ; `RosterListView.swift:refresh()` utilise désormais `entity.lastMessage` comme unique source de `model.message` (suppression de `pair.lastMessage?.message ?? entity.lastMessage`). Commit 6404681, CI run 32934697138 (succès). Test réel restant : vérifier sur device qu'une conversation avec plusieurs messages affiche bien le dernier envoyé/reçu dans la liste des conversations.
```
...
STATUT : BUILD_VALIDATED (2026-08-26) — Portée réduite documentée : option "à défaut" de la RECOMMANDATION appliquée (pas l'option complète URLSessionConfiguration(.background) + file d'attente Core Data persistée). `PublishComposeView.publish()` (Sources/TiinverSwift/Feed/PublishComposeView.swift) enveloppe désormais tout l'upload dans `UIApplication.shared.beginBackgroundTask(withName:expirationHandler:)`/`endBackgroundTask` (via `defer`), pour survivre à une mise en arrière-plan brève (~30s, prolongeable par iOS). La persistance/reprise après échec réseau complet (app tuée, backgroundTask expiré) N'EST PAS traitée — hors périmètre de ce correctif, qui cible le cas le plus fréquent (bascule brève vers une autre app / verrouillage d'écran pendant l'upload). Commit e1b8b0a, CI run 32935455105 (succès). Test réel restant : lancer une publication vidéo, basculer vers une autre app ou verrouiller l'écran pendant l'upload, confirmer que la publication aboutit malgré tout au retour dans l'app.
```
...
CONTRE-AUDIT : trouvé par l'agent "Pipeline média — Chat (image/vidéo/fichier/vocal)" (Phase A.2, 2026-08-24)
STATUT : BUILD_VALIDATED (2026-08-26) — Portée réduite documentée : seul le déclencheur reconnexion socket est reproduit (pas de scan périodique BGTaskScheduler ni de scan sur notification push, pas de politique retry/backoff dédiée). Ajouté `MessageRepository.pendingUploads(currentUsername:)` (scan `isFileUploaded==0 AND usernameFrom==currentUsername`, toutes conversations) et `ChatRepository.resumePendingUploads()` (appelé depuis `onConnected()`, ré-upload + ré-envoi socket pour chaque message trouvé). Risque de double upload/double envoi identifié (ce nouveau chemin peut désormais courir concurremment avec `ChatViewModel.requestUpload` pour le même message) et corrigé par un verrou synchrone partagé `ChatMediaUploadService.reserveUpload/releaseUpload` (sûr sans `actor` dédié car les deux appelants sont `@MainActor` et la réservation ne contient aucun `await` interne). Fichiers : `Sources/TiinverSwift/Storage/MessageRepository.swift`, `Sources/TiinverSwift/Realtime/ChatRepository.swift`, `Sources/TiinverSwift/Messagerie/ChatViewModel.swift`, `Sources/TiinverSwift/Messagerie/ChatMediaUploadService.swift`. Commit d5b583c, CI run 32937077298 (succès). Test réel restant : envoyer une pièce jointe chat, tuer l'app (ou couper le réseau) avant la fin de l'upload, rouvrir l'app SANS revenir sur la conversation, confirmer que l'upload/envoi reprend automatiquement à la reconnexion socket.
```
...
STATUT : BUILD_VALIDATED (2026-08-26) — RECOMMANDATION appliquée : `PlacedText.scale`/`rotation` ajoutés, nouvelle `PlacedItemView` (Sources/TiinverSwift/PhotoEditor/PhotoToolsView.swift) enveloppe chaque calque avec `DragGesture`+`MagnificationGesture`(bornée `MIN_SCALE`/`MAX_SCALE` = 0.3...5.0, valeurs EXACTES d'`ImageViewCanvas.java:73-74`, clampée EN TEMPS RÉEL comme `ScaleListener.onScale`)+`RotationGesture`, composés via `simultaneousGesture` pour reconnaissance simultanée (pincer+pivoter+glisser comme Android). `flatten()` applique `scaleEffect`/`rotationEffect` lors de la composition finale. Gestes désactivés pendant le mode peinture (`allowsHitTesting(!isDrawMode)`). Écart mineur assumé et documenté : pas de surbrillance de sélection dédiée (`objectInAction`), SwiftUI route déjà chaque geste au bon calque via son propre hit-testing — comportement observable équivalent. Commit df853b8, CI run 32938280334 (succès). Test réel restant : ajouter un texte/sticker, confirmer glisser/pincer-zoomer/pivoter fonctionnent indépendamment sur plusieurs calques superposés, confirmer le rendu final (`flatten()`) reflète fidèlement position/échelle/rotation.
```
